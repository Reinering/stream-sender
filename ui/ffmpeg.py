...
from .Ui_ffmpeg import Ui_MainWindow
    ...
    def run(self):
        if self.protocol == "UDP":
            if self.videoFormat == "MPEG4":
                self.__cmd = "ffmpeg -re -stream_loop -1 -i " + self.filePath + " -vcodec libx264 -acodec copy -f mpegts " + self.protocol.lower()+ "://" + self.multiCast_ip + ':' + self.port
            elif self.videoFormat == "TS":
                self.__cmd = "ffmpeg -re -stream_loop -1 -i " + self.filePath + " -vcodec copy -f mpegts " + self.protocol.lower() + "://" + self.multiCast_ip + ':' + self.port
        ...
        print(self.__cmd)
        self.p = subprocess.Popen(args=self.__cmd,
                                  stdin=subprocess.PIPE,
                                  stdout=subprocess.PIPE,
                                  stderr=subprocess.PIPE,
                                  shell=True)
        # self.p = subprocess.call(self.__cmd, stdin = subprocess.PIPE, stdout=subprocess.PIPE, stderr = subprocess.PIPE, shell=True)
        # print("Type", type(self.p))
        out = str(self.p.stdout.read(), encoding="utf-8")
        print('out:', out)
